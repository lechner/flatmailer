...
cli_option cli_options[] = {
  { 'a', "use-args", cli_option::flag, use_args, &use_recips, sizeof(use_recips),
    "Use only command-line arguments for recipients", 0 },
  { 'b', "use-both", cli_option::flag, use_both, &use_recips, sizeof(use_recips),
    "Use both command-line and message header for recipients", 0 },
  { 'e', "use-either", cli_option::flag, use_either, &use_recips, sizeof(use_recips),
    "Use either command-line and message header for recipients", 0 },
  { 'h', "use-header", cli_option::flag, use_header, &use_recips, sizeof(use_recips),
    "Use only message header for recipients", 0 },
  { 'f', "from", cli_option::string, 0, &o_from, sizeof(o_from),
    "Set the sender address", 0 },
  { 'n', "no-queue", cli_option::flag, 1, &show_message, sizeof(show_message),
    "Send the formatted message to standard output", 0 },
  { 'v', "show-envelope", cli_option::flag, 1, &show_envelope, sizeof(show_envelope),

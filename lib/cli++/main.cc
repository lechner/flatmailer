...
static cli_option help_option = { 'h', "help", cli_option::flag,
				  true, &do_show_usage, sizeof(do_show_usage),
...
void cli_option::assert_dereferenced_size(std::size_t target)
{
  if(dereferenced_size != target) {
    ferr << cli_program << "FATAL: Dereferenced pointer size not equal to target when setting option " << name << "." << endl;
    exit(1);
  }
}
...
int cli_option::set(const char* arg)
{
  ...
  switch(type) {
  case flag:
    assert_dereferenced_size(sizeof(int));
  ...
  case ulong:
    assert_dereferenced_size(sizeof(unsigned long));
    *(unsigned long*)dataptr = strtoul(arg, &endptr, 10);
    ...
    return 1;
  case stringlist:
    assert_dereferenced_size(sizeof(cli_stringlist*));
    *(cli_stringlist**)dataptr =
      stringlist_append(*(cli_stringlist**)dataptr, arg);
    return 1;
  default: // string
    assert_dereferenced_size(sizeof(char*));

...
cli_option cli_options[] = {
  { 0, "host", cli_option::string, 0, &remote, sizeof(remote),
    "Set the hostname for the remote", 0 },
  { 'p', "port", cli_option::integer, 0, &port, sizeof(port),
    "Set the port number on the remote host to connect to", 0 },
  { 0, "user", cli_option::string, 0, &user, sizeof(user),
    "Set the user name for authentication", 0 },
  { 0, "pass", cli_option::string, 0, &pass, sizeof(pass),
    "Set the password for authentication", 0 },
  { 0, "auth-login", cli_option::flag, AUTH_LOGIN, &auth_method, sizeof(auth_method),
    "Use AUTH LOGIN instead of auto-detecting in SMTP", 0 },
  { 0, "source", cli_option::string, 0, &source, sizeof(source),
    "Source address for connections", 0 },
#ifdef HAVE_TLS
  { 0, "tls", cli_option::flag, 1, &use_tls, sizeof(use_tls),
    "Connect using TLS (on an alternate port by default)", 0 },
  { 0, "ssl", cli_option::flag, 1, &use_tls, sizeof(use_tls),
    "Alias for --tls", 0 },
  { 0, "starttls", cli_option::flag, 1, &use_starttls, sizeof(use_starttls),
    "Use STARTTLS command", 0 },
  { 0, "x509certfile", cli_option::string, 0, &tls_x509certfile, sizeof(tls_x509certfile),
    "Client certificate file", 0 },
  { 0, "x509keyfile", cli_option::string, 0, &tls_x509keyfile, sizeof(tls_x509keyfile),
    "Client certificate private key file", "the same file as --x509certfile" },
  { 0, "x509cafile", cli_option::string, 0, &tls_x509cafile, sizeof(tls_x509cafile),
    "Certificate authority trust file", DEFAULT_CA_FILE },
  { 0, "x509crlfile", cli_option::string, 0, &tls_x509crlfile, sizeof(tls_x509crlfile),
    "Certificate revocation list file", 0 },
  { 0, "x509fmtder", cli_option::flag, true, &tls_x509derfmt, sizeof(tls_x509derfmt),
    "X.509 files are in DER format", "PEM format" },
  { 0, "insecure", cli_option::flag, true, &tls_insecure, sizeof(tls_insecure),
